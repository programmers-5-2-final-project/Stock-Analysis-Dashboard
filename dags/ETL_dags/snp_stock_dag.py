...
with DAG(
    dag_id="snp_dag17",  # dag 이름. 코드를 변경하시고 저장하시면 airflow webserver와 동기화 되는데, dag_id가 같으면 dag를 다시 실행할 수 없어, 코드를 변경하시고 dag이름을 임의로 바꾸신후 테스트하시면 편해요. 저는 dag1, dag2, dag3, ... 방식으로 했습니다.
    ...
    # )
    load_snp_stock_to_rds_from_s3(load_snp_stock_to_s3(transform_snp_stock(True)))

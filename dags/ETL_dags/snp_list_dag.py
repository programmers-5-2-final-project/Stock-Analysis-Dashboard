...
@task
def transform_snp_stock_list(
    _extract_snp_stock: bool,
) -> bool:  # 기업 단위로 추출한 주식 데이터 전처리 테스크
    """
    input: snp500의 심볼리스트
...
@task
def load_snp_stock_list_to_rds_from_s3(_load_snp_stock_to_s3: bool) -> bool:
    """
...
with DAG(
    dag_id="snp_stock_list_dag6",  # dag 이름. 코드를 변경하시고 저장하시면 airflow webserver와 동기화 되는데, dag_id가 같으면 dag를 다시 실행할 수 없어, 코드를 변경하시고 dag이름을 임의로 바꾸신후 테스트하시면 편해요. 저는 dag1, dag2, dag3, ... 방식으로 했습니다.
    ...
    # )
    load_snp_stock_list_to_rds_from_s3(

...
def transform_krx_list_data(task_logger):
    task_logger.info("Reading extracted data from csv")
    raw_df = csv_to_df(FilePath.tmp_krx_list_csv.value)

    task_logger.info("Dropping wrong row of which Code, Name are NaN")
    transformed_krx = Transform("KRX", raw_df)
    transformed_krx.drop_nan(["Code", "Name"])

    task_logger.info("Loading transformed data to csv")
    df_to_csv(transformed_krx.df, FilePath.data_krx_list_csv.value)

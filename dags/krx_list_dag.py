...
@task
def transform_krx_stock(extracted_krx_list): # 기업 단위로 추출한 주식데이터 전처리 테스크
    task_logger.info(f"Transform krx_list")
    raw_df = pd.read_csv(f"./tmp/krx_list.csv") # 저장된 데이터를 다시 DataFrame으로 불러옴
    transformed_df = transformed_df.dropna(subset=["Code", "Name"]) # Code나 Name에 Nan값이 있는 행은 제거
    transformed_df.to_csv(f"./data/krx_list.csv", index=False) # 다음 테스크로 데이터를 이동시키기 위해 csv 파일로 저장.
    return krx_list

...
def extract_krx_co_info():
    ...
    new_columns = ['Code', 'Name', 'Market', 'Sector', 'Industry', 'ListingDate', 'SettleMonth', 'Representative', 'HomePage', 'Region']
    df = pd.DataFrame(columns=new_columns)
    df.to_csv("./tmp/krx_co_info.csv", index=False, encoding="utf-8-sig") 
    raw_df.to_csv("./tmp/krx_co_info.csv", mode="a", index=False, header=False, encoding="utf-8-sig")
    return True
...
def transform_krx_co_info(_):
    ...
    transformed_df = raw_df.dropna(subset=["Code", "Name", "Sector"]) # Code나 Name에 Nan값이 있는 행은 제거
    new_columns = ['Code', 'Name', 'Market', 'Sector', 'Industry', 'ListingDate', 'SettleMonth', 'Representative', 'HomePage', 'Region']
    df = pd.DataFrame(columns=new_columns)
    df.to_csv("./data/krx_co_info.csv", index=False, encoding="utf-8-sig") 
    transformed_df.to_csv("./data/krx_co_info.csv", mode="a", index=False, header=False, encoding="utf-8-sig")
    return True
...
def load_krx_co_info_to_rds_from_s3(_):
    ...
    task_logger.info("Importing krx_co_info.csv data from Amazon S3 to RDS for PostgreSQL DB instance")                
    ...
    engine.execute(text("DELETE FROM raw_data.krx_co_info WHERE code like '%Code%';")) # 첫 행이 header여서 지워주는 쿼리
    engine.execute(text("""
...
with DAG(
    ...
    load_krx_co_info_to_rds_from_s3(load_krx_co_info_to_s3(transform_krx_co_info(extract_krx_co_info())))

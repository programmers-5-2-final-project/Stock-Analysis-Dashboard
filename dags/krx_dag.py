...
def extract_krx_list():  # KRX(코스피, 코스닥, 코스넷)에 상장되어 있는 현재 기업의 심볼을 추출 테스크
    ...
    krx_list_df.to_csv("./data/krx_list.csv", index=False, encoding="utf-8-sig") # 다음 테스크로 데이터를 이동시키기 위해 csv 파일로 저장

    return True

@task
def extract_krx_stock(_extract_krx_list, symbol): # 기업 단위로 주식데이터 추출 테스크
    task_logger.info(f"Extract krx_stock_{symbol}")
    raw_df = fdr.DataReader(symbol, "2003")
    raw_df.to_csv(f"./tmp/krx_stock_{symbol}.csv", index=True) # 다음 테스크로 데이터를 이동시키기 위해 csv 파일로 저장.
    return True

@task
def transform_krx_stock(_extract_krx_stock, symbol): # 기업 단위로 추출한 주식데이터 전처리 테스크
...
@task
def load_krx_stock_to_s3(_transform_krx_stock, symbol): # 기업 단위로 주식데이터 S3에 적재 테스크
    ...
    krx_stock_data = open(f"./data/krx_stock_{symbol}.csv", "rb") # 저장된 데이터를 다시 DataFrame으로 불러옴
    s3.Bucket("de-5-2").put_object(Key=f"krx_stock_{symbol}.csv", Body=krx_stock_data) # S3 버킷에 저장
    return True

@task
def load_krx_stock_to_rds_from_s3(_load_krx_stock_to_s3, symbol): # 기업 단위로 S3에 적재한 주식데이터를 RDS(DW)에 COPY 테스크
    ...
    task_logger.info(f"Importing krx_stock_{symbol}.csv data from Amazon S3 to RDS for PostgreSQL DB instance")                
    engine.execute(f"""
...
@task
def end(etl): # 종료를 알리는 테스크. 아무 기능 없음
    task_logger.info("End krx_dag")
    return

with DAG(
    dag_id="krx_dag39", # dag 이름. 코드를 변경하시고 저장하시면 airflow webserver와 동기화 되는데, dag_id가 같으면 dag를 다시 실행할 수 없어, 코드를 변경하시고 dag이름을 임의로 바꾸신후 테스트하시면 편해요. 저는 dag1, dag2, dag3, ... 방식으로 했습니다.
    ...
    _extract_krx_list = extract_krx_list() # KRX(코스피, 코스닥, 코스넷)에 상장되어 있는 현재 기업의 심볼을 추출 테스크 실행
    krx_list = pd.read_csv("./data/krx_list.csv") # 저장된 기업리스트 불러옴
    ...
    etl=[] # 여러 갈래로 나눠진 task를 end task에 묶기 위한 사전작업
    for symbol in krx_list: # ✨ 한번에 불러오는 데이터양 많으니 테스트 하실때는 for symbol in krx_list[:2]: 로 꼭 바꿔서 하세요!
        _extract_krx_stock = extract_krx_stock.override(task_id="extract_krx_stock_"+symbol)(_extract_krx_list, symbol)  #테스크간 의존성 설정. task_id는 기업심볼단위로 설정. _extract_krx_list같이 쓰진 않지만 의존성을 설정하기 위해 인자로 넣음
        ...
        etl.append(_load_krx_stock_to_rds_from_s3) # 여러갈래로 나눠진 task를 end task에 묶기
